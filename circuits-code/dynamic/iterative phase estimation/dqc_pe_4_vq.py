...
circuit.p(-np.pi / 2, qreg[1]).c_if(creg, 1)
circuit.p(-np.pi / 4, qreg[2]).c_if(creg, 1)
circuit.p(-np.pi / 8, qreg[3]).c_if(creg, 1)
circuit.h(qreg[1])
circuit.measure(qreg[1], creg[1])
circuit.p(-np.pi / 2, qreg[2]).c_if(creg, 2)
circuit.p(-np.pi / 4, qreg[3]).c_if(creg, 2)
circuit.h(qreg[2])
circuit.measure(qreg[2], creg[2])
circuit.p(-np.pi / 2, qreg[3]).c_if(creg, 4)
circuit.h(qreg[3])
